...
for word in sent_lst:
    # Beside checking the word in stopwords, checking if the word is not last in the list, so afterwards in the elif 
    # statement the last acronyme doesn't have .(dot) as the last character.
    if word not in stopwords and (sent_lst.index(word) != len(sent_lst) -1):
        acro += word[0:2].upper() + '. '
...
t_phrase = reversed(p_phrase)                       # Reversed function returns an object of class reversed, in which
r_phrase = ''                        
for char in t_phrase:                               # the program should iterate over and pass every character in 
    r_phrase += char                                # r_phrase string.
print(r_phrase)
# --------------------------------------------------------------------------------------------------------------------
...
# Storing every item detail in own list item_details, and accesing every detail to use afterwards in the format statement.
    ...
    print('The store has{} {}, each for{} USD.'.format(stock, itm, price))
